...
class MarkedCNN(torch.nn.Module):
    ...
    Parameters:
        ...
        activation :: str - activation function name
    '''
    def __init__(self, mask, dim, hdims=[], ksize=3, bdy_cond='reflect', bias=True, activation='Tanh', **kwargs):
        assert ksize is None or ksize%2==1, 'ksize={} must be an odd integer'.format(ksize)
        super().__init__()
        self.mask = mask
        d = self.mask.dim()
    ...
    def forward(self, x):
        ...
        x = torch.cat([x, self.mask[None,None,...]], axis=1) # concatenate mask as one additional feature 
        for layer in self.layers:
            x = layer(x)
        return x
